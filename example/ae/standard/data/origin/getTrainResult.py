...
def main():
    ...
    while 1:
        ...
        if not line:
            break
        namelist = []
        namelist.append(line.split('.'))  # 将文件名按照.符号进行分割

        if len(namelist[0][0].split(' ')) >= 2:  # 如果文件名的第一项用空格分割是个长度超过1的list，那么则可以认为该文件用空格分割
            namelist[0][0] = line.split(' ')  # 将文件名按照空格进行分割
            ...
            indexArray = [int(entityNum), int(Season), int(Year), int(Codex), int(Res)]
            writeArray = [0 for x in range(0, 6)]

            if indexArray[1] != '':  # Season输入
                writeArray[0] = line
                writeArray[1] = 'Season'
                writeArray[2] = ' '.join(namelist[:indexArray[0] + 1])
                writeArray[3] = 0
                writeArray[4] = namelist[indexArray[1]]
                writeArray[5] = 0  # 以累加的方式算offset
                for i in range(indexArray[1]):
            ...
            if indexArray[2] != '':  # Year输入
                ...
                writeArray[2] = ' '.join(namelist[:indexArray[0] + 1])
                writeArray[3] = 0
                writeArray[4] = namelist[indexArray[2]]
                writeArray[5] = 0  # 以累加的方式算offset
                for i in range(indexArray[2]):
                    writeArray[5] += len(namelist[i]) + 1
                writeArray[5] += 1  # 修正offset
                csv_write.writerow(writeArray)
            ...
            if indexArray[3] != '':  # Codex输入
                ...
                writeArray[2] = ' '.join(namelist[:indexArray[0] + 1])
                writeArray[3] = 0
                writeArray[4] = namelist[indexArray[3]]
                writeArray[5] = 0  # 以累加的方式算offset
                for i in range(indexArray[3]):
                    writeArray[5] += len(namelist[i]) + 1
                writeArray[5] += 1  # 修正offset
                csv_write.writerow(writeArray)
            ...
            if indexArray[4] != '':  # Res输入
                ...
                writeArray[2] = ' '.join(namelist[:indexArray[0] + 1])
                writeArray[3] = 0
                writeArray[4] = namelist[indexArray[4]]
                writeArray[5] = 0  # 以累加的方式算offset
                for i in range(indexArray[4]):
                    writeArray[5] += len(namelist[i]) + 1
                writeArray[5] += 1  # 修正offset
                csv_write.writerow(writeArray)
